...
make -j 4 && ./bin/FreeCADCmd -t femtest.app.test_solver_calculix.TestSolverCalculix.test_multibody_face_analysis
...
import unittest
unittest.TextTestRunner().run(unittest.TestLoader().loadTestsFromName(
    'femtest.app.test_solver_calculix.TestSolverCalculix.test_multibody_face_analysis'
))
